...
The `Release` Actions workflow needs six repository secrets: `MACOS_CERTIFICATE_P12_BASE64`, `MACOS_CERTIFICATE_PASSWORD`, `APP_STORE_CONNECT_KEY_P8_BASE64`, `APP_STORE_CONNECT_KEY_ID`, `APP_STORE_CONNECT_ISSUER_ID`, and `SPARKLE_PRIVATE_KEY`. These six secrets are configured. The five Apple credentials were transferred from Dayline using a temporary workflow that sealed them to this repository's Actions public key; only destination-encrypted ciphertext left the runner. The temporary source branch and run were removed, and Dayline main was unchanged. GitHub cannot read back secret plaintext, but this same encrypted transfer can bootstrap another explicitly authorized repository without asking Robin to re-enter credentials. Keep Sparkle keys separate. Never commit credentials. Choose **one submitter** per release: Actions when configured, otherwise the local command. Do not run both concurrently.
